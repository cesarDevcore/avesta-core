...
bool RpcServer::processJsonRpcRequest(const HttpRequest& request, HttpResponse& response) {
  ...
  try {
    ...
    static std::unordered_map<std::string, RpcServer::RpcHandler<JsonMemberMethod>> jsonRpcHandlers = {
      ...
      { "f_core_stats_json", { makeMemberMethod(&RpcServer::f_on_core_stats_json), false } },
...
bool RpcServer::f_on_core_stats_json(const F_COMMAND_RPC_GET_CORE_STATS::request& req, F_COMMAND_RPC_GET_CORE_STATS::response& res){
  Hash hash = m_core.getTopBlockHash();

  BlockTemplate blk = m_core.getBlockByHash(hash);
  BlockDetails blkDetails = m_core.getBlockDetails(hash);
  uint32_t height = boost::get<BaseInput>(blk.baseTransaction.inputs.front()).blockIndex;

  CryptoNote::CoreStatistics stats = m_core.getCoreStatistics(); 

  res.stats.transaction_pool_size = stats.transactionPoolSize; 
  res.stats.blockchain_height = stats.blockchainHeight; 
  res.stats.mining_speed = stats.miningSpeed; 
  res.stats.alternative_block_count = stats.alternativeBlockCount; 
  res.stats.top_block_hash_string = stats.topBlockHashString; 

  res.stats.difficulty = m_core.getBlockDifficulty(height);
  res.stats.alreadyGeneratedCoins = std::to_string(blkDetails.alreadyGeneratedCoins);
  res.stats.alreadyGeneratedTransactions = blkDetails.alreadyGeneratedTransactions;
  res.stats.maxBlockIndex = blkDetails.index;
...
bool RpcServer::on_getblocktemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request& req, COMMAND_RPC_GETBLOCKTEMPLATE::response& res) {
  ...
  AccountPublicAddress acc = boost::value_initialized<AccountPublicAddress>();
   std::string miner_address = DEFAULT_MINER_ADDRESS;


   if (!miner_address.size() || !m_core.getCurrency().parseAccountAddressString(miner_address, acc)) {
