...
struct f_core_stats_response {
  uint64_t transaction_pool_size;
  uint64_t blockchain_height;
  uint64_t mining_speed;
  uint64_t alternative_block_count;
  std::string top_block_hash_string;
  uint64_t difficulty;
  uint64_t alreadyGeneratedTransactions;
  std::string alreadyGeneratedCoins;
  ...
  void serialize(ISerializer &s) {
    KV_MEMBER(transaction_pool_size)
    KV_MEMBER(blockchain_height)
    KV_MEMBER(mining_speed)
    KV_MEMBER(alternative_block_count)
    KV_MEMBER(top_block_hash_string)
    KV_MEMBER(difficulty)
    KV_MEMBER(alreadyGeneratedTransactions)
    KV_MEMBER(alreadyGeneratedCoins)
    KV_MEMBER(maxBlockIndex)
  }
};

struct F_COMMAND_RPC_GET_CORE_STATS {
  typedef EMPTY_STRUCT request;

  struct response {
    f_core_stats_response stats;
    std::string status;
